...
async def version(ctx):
    ...
    embed.add_field(name="Version:", value="Current Version: **1.4** (BETA)", inline=False)
...
@client.command()
@commands.has_any_role('Developer', 'Owner', 'Admin', 'Staff Team')
async def kick(ctx, member : discord.Member, *, reason=None):
    global logchannel
    await member.kick(reason=reason)
    embed = discord.Embed(title="", color=0x00ff00)
    embed.add_field(name="", value=f"<:white_check_mark:> ***{member} was kicked***", inline=False)
    await ctx.send(embed=embed)
    embed = discord.Embed(title="", color=0x00ff00)
    embed.add_field(name="", value=f"<:white_check_mark:> ***{member} was kicked***", inline=False)
    embed.add_field(name="Trigger / Staff", value=f"Username: {ctx.author}\nUser ID: {ctx.author.id}", inline=False)
    await logchannel.send(embed=embed)
...
async def ban(ctx, member : discord.Member, *, reason=None):
    ...
    embed = discord.Embed(title="", color=0x00ff00)
    embed.add_field(name="", value=f"<:white_check_mark:> ***{member} was banned***", inline=False)
    await ctx.send(embed=embed)
    embed.add_field(name="", value=f"<:white_check_mark:> ***{member} was banned***", inline=False)
    embed.add_field(name="Trigger / Staff", value=f"Username: {ctx.author}\nUser ID: {ctx.author.id}", inline=False)
    await logchannel.send(embed=embed)
...
async def unban(ctx, *, member):
    ...
    for ban_entry in banned_users:
        ...
        if (user.name, user.discriminator) == (member_name, member_discriminator):
            ...
            embed = discord.Embed(title="", color=0x00ff00)
            embed.add_field(name="", value=f"<:white_check_mark:> ***{member} was unbanned***", inline=False)
            await ctx.send(embed=embed)
            embed.add_field(name="", value=f"<:white_check_mark:> ***{member} was unbanned***", inline=False)
            embed.add_field(name="Trigger / Staff", value=f"Username: {ctx.author}\nUser ID: {ctx.author.id}", inline=False)
            await logchannel.send(embed=embed)
...
async def online(ctx):
    channel = client.get_channel(710428814227931146)
    chtopic = channel.topic
    cutchtopic = chtopic[0:21]
    embed = discord.Embed(title="Who is Online on TPNet?", color=0x00ff00)
    embed.add_field(name="Who is Online on TPNet?", value=f"{cutchtopic}", inline=False)
    await ctx.send(embed=embed)
